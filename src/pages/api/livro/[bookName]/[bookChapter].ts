  ...
  if (req.method !== 'GET') {
    ...
    books.forEach((book: BooksEntity, idx: number) => {
      ...
      if (jsonBookName === bookName){
        ...
        console.log('>> idx: ' + idx);
        console.log('>> number: ' + bookChapter);
      }
    });
    
    if (typeof bookIndex !== 'undefined') {
      const bookData = books[bookIndex];
      if (isValid(bookChapter, bookData)) {
        const chapterData = books[bookIndex].chapters[bookChapter - 1];
        ...
        res.status(502).json({message: `O capítulo ${bookChapter} não existe`})
      }
    } else {
